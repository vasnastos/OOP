...
void Racing::reset_race()
{
    ...
    this->position.clear();
    vector <string> names{"Blitz","Bolt","Goliath","Hercules","Rogue","Danger","Raider","Storm","Nitro","Hulk"};
    shuffle(names.begin(),names.end(),default_random_engine(steady_clock::now().time_since_epoch().count()));
    auto rand_int=uniform_int_distribution<int>(50,100);
    for(int i=0;i<this->n_horses;i++)
    {
        horse ahorse(i+1,names.at(i),rand_int(mt),rand_int(mt),rand_int(mt));
...
{
    // return count_if(this->standings.begin(),this->standings.end(),[](pair <horse,int> &p) {return p.second==DNF;})==0;
    return this->standings.size()==this->n_horses;
}
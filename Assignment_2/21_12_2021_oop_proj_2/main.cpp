...
public:
    ...
    void showlessons(wxCommandEvent& ev)
    {
        ...
        std::string message = studentitr->get_id() + " " + studentitr->get_name() + " " + to_string(studentitr->get_semester()) + " " + to_string(studentitr->get_credits());
        wxMessageBox(wxString(wxString::FromUTF8(message)), wxT("ΣΤΟΙΧΕΙΑ ΦΟΙΤΗΤΗ"), wxICON_INFORMATION);
        //0-->Αρχή της λίστας
